 ...
 *
 *  --- PATCHED for crime-aware routing ---
 *  Adds a "crime_aware" weighting case immediately before the final throw.
 *  All original GH 9.1 logic is preserved verbatim.
 *  Source: commit 73e6b7cc3ca163ce0b53692f7cd732dba170bfce
 */
...
public class DefaultWeightingFactory implements WeightingFactory {
    ...
    public Weighting createWeighting(Profile profile, PMap requestHints, boolean disableTurnCosts) {
        // Merge profile hints with request hints; request hints take precedence.
        PMap hints = new PMap();
        ...
        if (profile.hasTurnCosts() && !disableTurnCosts) {
            ...
            turnCostProvider = new DefaultTurnCostProvider(
                turnRestrictionEnc, graph.getTurnCostStorage(), uTurnCosts);
        } else {
        ...
        if (CustomWeighting.NAME.equalsIgnoreCase(weightingStr)) {
            ...
            throw new IllegalArgumentException(
                "Instead of weighting=shortest use weighting=custom "
                + "with a high distance_influence");
        } else if ("fastest".equalsIgnoreCase(weightingStr)) {
            throw new IllegalArgumentException(
                "Instead of weighting=fastest use weighting=custom "
                + "with a custom model that avoids road_access == DESTINATION");
        } else if ("curvature".equalsIgnoreCase(weightingStr)) {
            throw new IllegalArgumentException(
                "The curvature weighting is no longer supported since 7.0. "
                + "Use a custom model with the EncodedValue 'curvature' instead");
